...
extern int GET_VARNAME_ONETIME;
extern char the_clean_text[30];
extern char MEI_STATUS[30];

void get_varname(void)

{
	    tx_packet[0] = '\x02';//|
		tx_packet[1] = '\x08';//|
		tx_packet[2] = '\x60';//|
		tx_packet[3] = '\x00';//|---------Get Variant Name Command
		tx_packet[4] = '\x00';//|
		tx_packet[5] = '\x08';//|
		tx_packet[6] = '\x03';//|
		tx_packet[7] = '\x60';//|
		...
		sprintf(log_message,"MEI Response Get Variant Name = %s",the_clean_text);
