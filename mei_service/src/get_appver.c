...
extern int GET_APPVER_ONETIME;
extern char the_clean_text[30];

void get_appver(void)

{
		...
		sprintf(log_message,"MEI Response App Version = %s",the_clean_text);
		...
		GET_APPVER_ONETIME = 1;    //Keeps this from spinning out of control
		tx_packet[2] = '\x10';    //Put tx_packet back in command mode
